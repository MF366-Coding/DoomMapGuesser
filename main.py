...
img_label = tk.Label(f3, text='Image not available.')
...
cur_screenshot = None
cur_settings = [GAME_PH, EPISODE_PH, MAP_PH, SECRETS_PH]
cur_selections = cur_settings.copy()
...
def choose_game_window(origin: tk.Button, master: tk.Tk | tk.Toplevel = root):
    def _save_changes():
        try:
    ...
    game_listbox = tk.Listbox(game_choice_win, listvariable=game_var, bg='dark blue', fg='yellow', selectmode=tk.SINGLE)
    accept_butt = tk.Button(game_choice_win, text='Confirm', command=lambda:
        _save_changes)
...
def choose_episode_window(origin: tk.Button, master: tk.Tk | tk.Toplevel = root):
    ...
    episode_listbox = tk.Listbox(episode_choice_win, listvariable=episode_var, bg='dark blue', fg='yellow', selectmode=tk.SINGLE)
    accept_butt = tk.Button(episode_choice_win, text='Confirm', command=lambda:
        _save_changes)
...
def choose_map_window(origin: tk.Button, master: tk.Tk | tk.Toplevel = root):
    ...
    maps_listbox = tk.Listbox(map_choice_win, listvariable=maps_var, bg='dark blue', fg='yellow', selectmode=tk.SINGLE)
    accept_butt = tk.Button(map_choice_win, text='Confirm', command=lambda:
        _save_changes)
...
choose_game_butt = tk.Button(f4, text=cur_selections[0], command=lambda:
    choose_game_window(choose_game_butt))
choose_episode_butt = tk.Button(f4, text=cur_selections[1], command=lambda:
    choose_episode_window(choose_episode_butt))
choose_map_butt = tk.Button(f4, text=cur_selections[2], command=lambda:
...
def display_intro(master: tk.Tk | tk.Toplevel = root):
    ...
    logo_label = tk.Label(intro_win, image=intro_win.tk_logo)
    ...
    about_4 = tk.Label(intro_win, text="The number of secrets are the number of sectors with Effect 9, whether they're acessible or not.")
...
def display_screenshot(screenshot_link: str):
    img = Image.frombytes('r', (350, 250), scrapper.scrape_byte_contents(screenshot_link))
...
secrets_label = tk.Label(f4, text=number_of_secrets_var)
secrets_plus_butt = tk.Button(f4, text='+', fg='red')
secrets_minus_butt = tk.Button(f4, text='-', fg='green')
...
generate_butt = tk.Button(f5, text="Generate new screenshot (WON'T ASK FOR CONFIRMATION!)", command=generate_new_game)
guess_butt = tk.Button(f5, text="Confirm guess (WON'T ASK FOR CONFIRMATION!)", command=guess_screenshot)
leave_butt = tk.Button(f5, text='Exit', fg='red', command=prevent_from_leaving)
...
generate_butt.grid(column=0, row=0)
guess_butt.grid(column=1, row=0)
leave_butt.grid(column=2, row=0)
