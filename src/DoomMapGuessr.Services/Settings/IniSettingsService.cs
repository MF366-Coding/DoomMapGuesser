...
namespace DoomMapGuessr.Services.Settings
{
		...
		IAsyncDisposable
	{
		...
		/// <inheritdoc/>
		public event SettingsChangedEventHandler<object?>? SettingsChanged = null;
		...
		)
		{
			...
			if (Contains(key))
				throw new ArgumentException("Key already has a value associated to it.");
		...
		private bool Contains(
			string[] keySplit, string key
					   ...
					   : keySplit[1] == "?"
						   ...
						   actualIni.Sections.ContainsSection(keySplit[0])
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
		...
		)
		{
			...
			if (split[1] == "*")
			{
				...
				// in here we use * cuz it means ALL
				SettingsChanged?.Invoke(new(split[0], null, null));
			...
			else
			{
				...
				if (Contains(split, key))
					oldValue = GetString(key);

				string? newValue = value is null ? "null" : value.ToString();

				actualIni[split[0]][split[1]] = newValue;
				SettingsChanged?.Invoke(new(key, oldValue, newValue));
