...
namespace DoomMapGuessr
{
	...
	internal sealed class Program
	{
		...
		private static void PrepareApplicationSettingsInternally(ISettingsService settings)
		{
			...
			string? blurEffects = settings.GetString("GUI.BlurEffects");

			if (!settings.Contains("GUI.BlurEffects") || (blurEffects != FALSE && blurEffects != TRUE))
				settings.Set("GUI.BlurEffects", 1);
